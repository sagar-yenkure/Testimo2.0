...
const timelineItemVariants = cva("grid items-center gap-x-4 gap-y-1", {
  variants: {
    status: {
      done: "text-primary",
...
const timelineDotVariants = cva(
  "timeline-dot col-start-2 col-end-3 row-start-1 row-end-1 flex size-4 shrink-0 items-center justify-center rounded-full border border-current",
  {
    variants: {
      status: {
        default: "[&>*]:hidden",
        ...
        custom:
          "[&>*:not(:nth-child(4))]:hidden [&>*:nth-child(4)]:block border-current bg-background",
      },
    },
    defaultVariants: {
      status: "default",
...
const timelineHeadingVariants = cva(
  "row-start-1 row-end-1 line-clamp-2 max-w-full",
  {
    variants: {
      side: {
        right: "col-start-3 col-end-4 mr-auto text-left",
