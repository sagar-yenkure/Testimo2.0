...
import { motion, useInView, useAnimation, useSpring, Variants } from "framer-motion";
  ...
  useEffect(() => {
    const unsubscribe = spring.on("change", (latest) => {
      if (ref.current) {
        ...
        ref.current.textContent = `${prefix}${formattedValue}${postfix}`;
      }
    });
    return () => unsubscribe();
  }, [prefix, postfix, spring]);

  // Ensure value displays even on initial render
  React.useEffect(() => {
    if (ref.current && !isInView) {
      ref.current.textContent = `${prefix}${value}${postfix}`;
    }
  }, [value, prefix, postfix, isInView]);
